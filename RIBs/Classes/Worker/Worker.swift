...
open class Worker: Working {
  ...
  public final var isStarted: Bool {
    do {
      return try self.isStartedSubject.value()
    } catch {
      return false
    }
  }
  ...
  public final func start(_ interactorScope: InteractorScope) {
    ...
    self.isStartedSubject.onNext(true)
    ...
    self.bind(to: weakInteractorScope)
  }
  ...
  public final func stop() {
    ...
    self.isStartedSubject.onNext(false)
  ...
  private func bind(to interactorScope: InteractorScope) {
    self.unbindInteractor()

    self.interactorBindingDisposable = interactorScope.isActiveStream
  ...
  private func unbindInteractor() {
    self.interactorBindingDisposable?.dispose()
    self.interactorBindingDisposable = nil
  }
...
private class WeakInteractorScope: InteractorScope {
  ...
  var isActive: Bool {
    return self.sourceScope?.isActive ?? false
  }

  var isActiveStream: Observable<Bool> {
    return self.sourceScope?.isActiveStream ?? Observable.just(false)
  }
