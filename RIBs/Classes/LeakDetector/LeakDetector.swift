...
public class LeakDetector {
  ...
  static var disableLeakDetectorOverride: Bool = false
    ...
    func reset() {
      self.trackingObjects.removeAllObjects()
      self.expectationCount.accept(0)
    }
  #endif
...
private class LeakDetectionHandleImpl: LeakDetectionHandle {
  ...
  var cancelled: Bool {
    return self.cancelledRelay.value
  }
  ...
  func cancel() {
    self.cancelledRelay.accept(true)
    self.cancelClosure?()
  }
}
