...
open class Interactor: Interactable {
  ...
  public final var isActive: Bool {
    do {
      return try self.isActiveSubject.value()
    } catch {
      return false
    }
  }
  ...
  public final var isActiveStream: Observable<Bool> {
    return self.isActiveSubject.asObservable().distinctUntilChanged()
  }
  ...
  public final func activate() {
    ...
    self.activenessDisposable = CompositeDisposable()

    self.isActiveSubject.onNext(true)
  ...
  public final func deactivate() {
    ...
    self.activenessDisposable?.dispose()
    self.activenessDisposable = nil

    self.isActiveSubject.onNext(false)
  }
