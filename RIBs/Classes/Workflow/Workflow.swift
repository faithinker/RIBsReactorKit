...
open class Workflow<ActionableItemType> {
  ...
  public final func onStep<NextActionableItemType,
    NextValueType>(
    _ onStep: @escaping (ActionableItemType) -> Observable<(
      NextActionableItemType,
      NextValueType
    )>
  )
  -> Step<ActionableItemType, NextActionableItemType, NextValueType> {
    return Step(workflow: self, observable: self.subject.asObservable().take(1))
  ...
  public final func subscribe(_ actionableItem: ActionableItemType) -> Disposable {
    ...
    self.subject.onNext((actionableItem, ()))
    return self.compositeDisposable
  }
  ...
  fileprivate func didCompleteIfNotYet() {
    ...
    guard !self.didInvokeComplete else {
      return
    }
    self.didInvokeComplete = true
    self.didComplete()
  }
}
...
open class Step<WorkflowActionableItemType, ActionableItemType, ValueType> {
  ...
  public final func onStep<NextActionableItemType,
    NextValueType>(
    _ onStep: @escaping (ActionableItemType, ValueType)
      -> Observable<(NextActionableItemType, NextValueType)>
  )
  -> Step<WorkflowActionableItemType, NextActionableItemType, NextValueType> {
    let confinedNextStep = self.observable
      ...
      .take(1)
      .flatMapLatest { (_, actionableItem: ActionableItemType, value: ValueType) -> Observable<(
        NextActionableItemType,
        NextValueType
      )> in
  ...
  /// - returns: This step.
  public final func onError(_ onError: @escaping ((Error) -> ()))
  -> Step<WorkflowActionableItemType, ActionableItemType, ValueType> {
    self.observable = self.observable.do(onError: onError)
    return self
  }
  ...
  public final func commit() -> Workflow<WorkflowActionableItemType> {
    ...
    let disposable = self.observable
      .do(onError: self.workflow.didReceiveError, onCompleted: self.workflow.didCompleteIfNotYet)
      .subscribe()
    _ = self.workflow.compositeDisposable.insert(disposable)
...
extension ObservableType {
  ...
  public func fork<WorkflowActionableItemType, ActionableItemType,
    ValueType>(_ workflow: Workflow<WorkflowActionableItemType>)
  -> Step<WorkflowActionableItemType, ActionableItemType, ValueType>? {
...
extension Disposable {
  ...
  public func disposeWith<ActionableItemType>(worflow: Workflow<ActionableItemType>) {
    self.disposeWith(workflow: worflow)
  }
}
