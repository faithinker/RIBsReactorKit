...
open class MultiStageComponentizedBuilder<Component, Router, DynamicBuildDependency>: Buildable {
  ...
  public var componentForCurrentBuildPass: Component {
    if let currentPassComponent = currentPassComponent {
      ...
      let currentPassComponent = self.componentBuilder()

      // Ensure each invocation of componentBuilder produces a new
      // component instance.
      let newComponent = currentPassComponent as AnyObject
      if self.lastComponent === newComponent {
        assertionFailure(
          "\(self) componentBuilder should produce new instances of component when build is invoked."
        )
      }
      self.lastComponent = newComponent
  ...
  public final func finalStageBuild(withDynamicDependency dynamicDependency: DynamicBuildDependency) -> Router {
    let router = self.finalStageBuild(with: self.componentForCurrentBuildPass, dynamicDependency)
    defer {
  ...
  ()>
{
  ...
  override public final func finalStageBuild(with component: Component, _ dynamicDependency: ()) -> Router {
    return self.finalStageBuild(with: component)
  }
  ...
  public final func finalStageBuild() -> Router {
    return self.finalStageBuild(withDynamicDependency: ())
  }
}
